...
export default function XiioWordmark({ className = "" }: Props) {
  return (
    ...
    >
      ...
      <span className="inline-flex h-[1em] items-center gap-[0.21em] shrink-0">
        <span className="w-[0.1em] h-full bg-current rounded-full" />
        <span className="w-[0.1em] h-full bg-current rounded-full" />
      </span>
      <span className="ml-[0.32em] shrink-0 size-[1em] rounded-full border-[0.1em] border-current box-border" />
    </span>
  );
}
