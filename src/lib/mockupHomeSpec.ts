export const MOCKUP_HOME = {
  pageShell: "w-full",
  contentRightPad: "lg:pr-[76px]",
  topBarHeight: "h-[60px]",
  searchBar: "w-[520px] h-[40px] text-[14px]",
  searchIconLeft: "left-[16px]",
  heroSection: "min-h-[488px]",
  heroInnerMinHeight: "min-h-[428px]",
  heroContentTop: "pt-[36px]",
  heroTextBottom: "pb-[40px]",
  heroToFeaturedHeader: "mt-[32px]",
  heroGrid: "lg:grid lg:items-end lg:grid-cols-[376px_1fr] lg:gap-24",
  heroTitle: "font-serif text-[48px] font-normal leading-[1.08] tracking-tight mb-4",
  heroSubtitle: "text-[14px] leading-[1.5] max-w-[376px] text-white/55 mb-7",
  ctaRow: "gap-[12px]",
  ctaButton: "h-[40px] px-6 text-[14px] rounded-full",
  featuredLabel:
    "text-[10px] font-semibold tracking-[0.2em] text-sky-400 uppercase mb-2",
  featuredTitle: "text-[14px] font-semibold text-white",
  featuredMeta: "text-[12px] text-white/45 mb-3",
  featuredPlay:
    "shrink-0 w-10 h-10 rounded-full border border-white/25 flex items-center justify-center text-white/90 hover:bg-white/10 transition",
  sectionTitle: "text-[14px]",
  sectionChevron: "w-3.5 h-3.5",
  viewAllLink: "text-xs",
  featuredHeaderToCards: "mt-3.5",
  sectionGap: "gap-8",
  featuredCardWidth: "w-[233px]",
  surfaceCardWidth: "w-[177px]",
  featuredRowWidth: "w-[1221px]",
  selectsRowWidth: "w-[974px]",
  featuredRowGap: "gap-[14px]",
  surfaceRowGap: "gap-3",
  campusBanner: "w-[380px] min-h-[190px]",
  cardRadius: "rounded-xl",
  accentBlue: "#7EC8E3",
  accentBlueBright: "#6EB5FF",
  bg: "#05070A",
} as const;

export const HOME_IMAGE_BASE = "/images/home";
