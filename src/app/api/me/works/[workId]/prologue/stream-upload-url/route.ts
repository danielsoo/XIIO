...
import { beginPrologueStreamUpload } from "@/lib/server/staging-stream-upload";
...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    const result = await beginPrologueStreamUpload(db, session.uid, workId);
    return NextResponse.json({ workId, ...result });
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (msg === "not_found") return jsonError("not_found", "작품을 찾을 수 없습니다.", 404);
    if (msg === "staging_incomplete" || msg === "prologue_staging_missing") {
      return jsonError("staging_incomplete", "스테이징된 프롤로그 영상이 없습니다.", 400);
    }
