...
export async function POST(request: Request, { params }: Params) {
  ...
  const existing = prologueSnap.exists
    ? parsePrologueDoc(prologueSnap.data() as Record<string, unknown>)
    : null;
  ...
  if (isRevision) {
    if (existing?.platformStatus !== "published") {
      return jsonError("invalid_state", "게시된 프롤로그만 수정본 영상을 올릴 수 있습니다.", 400);
    }
    ...
    return jsonError("prologue_locked", "심사 중인 프롤로그 영상은 교체할 수 없습니다.", 403);
  }

  const oldUid = isRevision ? existing?.pendingRevision?.streamUid : existing?.streamUid;
  if (oldUid) {
    try {
