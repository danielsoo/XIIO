...
영화·방송 업계에 있는 모든 사람 — 감독·배우·스태프·창작자 — 을 위한 **영상 업로드 + 전문 네트워크 플랫폼**입니다. 한마디로 **"영화계의 LinkedIn"** 을 지향합니다. 대학생은 그중 한 세그먼트이며, 이들을 위해 학교를 대표하는 크리에이터로 소개되는 **특별한 창구**를 별도로 두고 있습니다.
...
영화·방송 업계 종사자·창작자는 만들 곳(장비·팀)은 있어도, **보여줄 곳과 이어질 곳**이 마땅치 않습니다. XIIO는 그 둘을 한 플랫폼에서 해결하는 걸 목표로 합니다 — **업로드가 곧 이력이 되고, 이력이 곧 네트워크가 되는 곳**.

1. **보여줄 곳** — 영화·예능·시리즈를 정식으로 업로드·시청할 수 있는 스트리밍 공간.
2. **이어질 곳** — 업로드가 곧 포트폴리오가 되고(크레딧·협업 기록 자동 축적), 그 포트폴리오를 바탕으로 다른 창작자·팀과 협업하거나, 채용/구직 제안을 주고받는 전문 네트워크.

핵심 유저는 업계 전반(감독·배우·스태프·프로덕션 등)이고, **대학생은 별도의 특별 창구(학교 태깅·학교 프로필·랭킹·월간 대표작)로 대표되는 하나의 세그먼트**입니다 — XIIO 전체가 "대학생용"인 것은 아닙니다.

## 핵심 컨셉

| 컨셉 | 무엇을 의미하나 |
|------|------|
| **작품이 곧 포트폴리오** | 감독·배우·스태프 크레딧이 자동으로 프로필 포트폴리오에 쌓임(`docs/product-network.md`) — 업계 종사자 누구에게나 적용되는 핵심 축, 별도로 이력서를 만들 필요가 없음 |
| **대학생 전용 창구** | 업로드 시 학교 태깅 → 학교 프로필·랭킹(`/schools`)·월간 대표작 자동 선정. 전체 플랫폼의 정체성이 아니라, 학생 세그먼트를 위한 특별 기능 |
...
지금은 "업계 종사자·창작자가 업로드하고, 그 기록이 포트폴리오가 되어 다른 사람과 1:1 혹은 그룹으로 협업·소통한다"는 기본 골격이 갖춰진 단계입니다. 다음 방향으로 다듬어가는 중입니다:
...
- **업계 네트워크로서의 발견/매칭 고도화** — 구직 제안(Business Invite) 게이트와 그룹채팅(Rooms)은 만들어졌고, 포트폴리오 기반 발견·매칭(LinkedIn처럼 사람·경력·작품을 찾고 이어주는 기능)을 더 정교하게 만드는 게 다음 단계입니다.
- **대학생 창구 강화** — 학교 랭킹·월간 대표작은 학생 세그먼트를 위한 별도 기능으로 계속 다듬어가되, 플랫폼 전체의 정체성으로 확대하지는 않습니다.
